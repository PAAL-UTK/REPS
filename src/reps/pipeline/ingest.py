...
def ingest_subject(subject_id: str, /) -> None:
    ...
    lbl_struct = root / "exercise_labels" / "structured" / f"REPS-{subject_id}_labels.parquet"
    lbl_unstruct = root / "exercise_labels" / "unstructured" / f"REPS-{subject_id}_labels.parquet"
    ...
    df = (
        ...
        .select(["ts", "ax", "ay", "az", "gx", "gy", "gz", "id"])  # ← keep only expected cols
    )
